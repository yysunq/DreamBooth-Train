...
if [[ -z "${reg_data_root}" ]]
then
        reg_data_root="/root/autodl-tmp/dreambooth/Dreambooth-Stable-Diffusion/regularization_images/costume"
fi
...
echo $time
finalpath="trained_models/${time}-${proj_name}-token(${token})-class_word(${class_word})-steps(${steps})-desc(${desc}).ckpt"
